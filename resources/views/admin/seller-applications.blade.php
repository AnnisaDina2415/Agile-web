        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <img src="{{ asset('storage/' . $app->ktp_image) }}" alt="KTP" class="max-w-xs rounded-xl border border-slate-200 shadow-sm">
                </div>
                ...
                @if ($app->isPending())
                    <div class="flex gap-3">
                        <form action="{{ route('admin.seller-applications.approve', $app) }}" method="POST" class="inline">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-green-500 hover:bg-green-600 text-white text-sm font-semibold transition-colors"
                        ...
                        <button type="button" onclick="showRejectModal({{ $app->id }})" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-semibold transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="rejectModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-2xl p-6 max-w-md w-full border border-slate-200 shadow-lg">
        <h3 class="text-lg font-bold text-slate-800 mb-4">Alasan Penolakan</h3>
        <form id="rejectForm" method="POST">
            @csrf
            @method('PATCH')
            <textarea name="rejection_reason" required rows="4"
                class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 mb-4"
                placeholder="Jelaskan alasan penolakan..."></textarea>
            <div class="flex gap-3">
                <button type="submit" class="flex-1 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-semibold text-sm transition-colors">
                    Tolak
                </button>
                <button type="button" onclick="closeRejectModal()" class="flex-1 bg-slate-200 hover:bg-slate-300 text-slate-800 px-4 py-2 rounded-lg font-semibold text-sm transition-colors">
                    Batal
                </button>
            </div>
        </form>
